...
import { resolveConfig, validateEnvFileKeys } from "../utils";
...
async function main() {
    ...
    console.log(`🔧 Generating configuration for environment: ${envArg}\n`);

    // Load the values file for this environment
    const valuesPath = join("config", `${envArg}.ts`);
    if (!existsSync(valuesPath)) {
        console.error(`❌ Error: ${valuesPath} does not exist.`);
        console.error(`\n💡 Tip: Copy config/example.ts to ${valuesPath} and fill in your values.`);
        process.exit(1);
    }
    ...
    try {
        ...
        console.error(`❌ Error loading ${valuesPath}:`, error);
        process.exit(1);
    }
    ...
    for (const [envfileName, envfileConfig] of Object.entries(schema.envfiles)) {
        ...
        if (existsSync(destPath)) {
            ...
            console.log(`📦 Backed up ${destPath} to ${backupPath}`);
        }
        ...
        // Add aliased keys (source key → target key name)
        ...
        console.log(`✅ Generated ${destPath} (${totalKeys} keys)`);
    }
    ...
    if (schema.configs) {
        ...
        for (const [configName, configDef] of Object.entries(schema.configs)) {
            ...
            if (!existsSync(configPath)) {
                console.log(`⚠️  Skipping ${configPath} (file does not exist)`);
                continue;
            }
            ...
            console.log(`📦 Backed up ${configPath} to ${backupPath}`);
            ...
            if (configDef.format === "toml") {
                ...
                console.log(`⚠️  Skipping ${configPath} (format ${configDef.format} not yet supported)`);
            }
        }
    }

    console.log(`\n🎉 Successfully generated configuration for ${envArg} environment!`);
}
