...
// Simple function-based API - just like TanStack Query expects
...
// ---------- Legacy Controller Factory (for advanced use cases) ----------
 ...
 *   name: 'todos',
 *   queryFn: () => fetchWithAuth(token, '/todos'),
 *   mutations: {
 *     create: (data) => fetchWithAuth(token, '/todos', { method: 'POST', body: JSON.stringify(data) }),
 *     update: ({ id, data }) => fetchWithAuth(token, `/todos/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
 *     remove: (id) => fetchWithAuth(token, `/todos/${id}`, { method: 'DELETE' }),
 *   }
 * });
 * ```
 * 
...
>(config: AdvancedOptimisticConfig<TApiData, TUiData, TStore>) {
  // Return a React hook that manages the store instance
  return function useAdvancedOptimisticStore() {
    // Use provided store or create from class or default
    const storeRef = React.useRef<TStore | null>(null);
    if (!storeRef.current) {
      storeRef.current = config.store || 
        (config.storeClass ? new (config.storeClass as any)() : new OptimisticStore<TUiData>()) as TStore;
    }
    
    // Infer the create and update types from the API interface
    type TCreate = InferCreateType<typeof config.api>;
    type TUpdate = InferUpdateType<typeof config.api>;
    
    return createEntityController<TApiData, TUiData, TCreate, TUpdate, TStore>({
      queryKey: [config.name],
      api: config.api,
      store: storeRef.current,
