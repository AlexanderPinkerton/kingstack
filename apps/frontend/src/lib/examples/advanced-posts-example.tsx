...
import { createOptimisticStore, OptimisticStore, DataTransformer, Entity } from "@/lib/optimistic-store-pattern";
...
export class PostStore extends OptimisticStore<PostUiData> {
  ...
  private _sortBy: 'newest' | 'oldest' | 'title' = 'newest';
  ...
  setSortBy(sort: 'newest' | 'oldest' | 'title') { this._sortBy = sort; }
  ...
  get filteredPosts(): PostUiData[] {
    ...
    posts.sort((a, b) => {
      switch (this._sortBy) {
        case 'newest': return b.created_at.getTime() - a.created_at.getTime();
        case 'oldest': return a.created_at.getTime() - b.created_at.getTime();
  ...
  get averageReadingTime(): number {
    ...
    return posts.reduce((total, post) => total + post.readingTime, 0) / posts.length;
  }
...
export class PostTransformer implements DataTransformer<PostApiData, PostUiData> {
  ...
  toUi(apiData: PostApiData): PostUiData {
    ...
    const author = apiData.author || (this.currentUser ? {
      id: this.currentUser.id,
      username: this.currentUser.user_metadata?.username || this.currentUser.email?.split('@')[0] || 'You',
...
function useAdvancedPosts() {
  ...
  return createOptimisticStore<PostApiData, PostUiData, PostStore>({
    name: 'advanced-posts',
    queryFn: () => fetchWithAuth(token, `${baseUrl}/posts`).then(res => res.json()),
    mutations: {
      create: (data) => fetchWithAuth(token, `${baseUrl}/posts`, {
        method: 'POST',
        body: JSON.stringify(data),
      }).then(res => res.json()),
      
      update: ({ id, data }) => fetchWithAuth(token, `${baseUrl}/posts/${id}`, {
        method: 'PUT', 
        body: JSON.stringify(data),
      }).then(res => res.json()),
      
      remove: (id) => fetchWithAuth(token, `${baseUrl}/posts/${id}`, {
        method: 'DELETE',
      }).then(() => ({ id })),
    },
    transformer: new PostTransformer(currentUser),
  ...
  if (status.isLoading) {
    return (
      <div className="text-center py-8">
        <div className="animate-pulse text-slate-300">Loading advanced posts...</div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-slate-800/50 border border-slate-600/50 rounded-lg p-4">
          <div className="text-2xl font-bold text-purple-300">{store.count}</div>
          <div className="text-sm text-slate-400">Total Posts</div>
        </div>
        <div className="bg-slate-800/50 border border-slate-600/50 rounded-lg p-4">
          <div className="text-2xl font-bold text-green-300">{store.publishedCount}</div>
          <div className="text-sm text-slate-400">Published</div>
        </div>
        <div className="bg-slate-800/50 border border-slate-600/50 rounded-lg p-4">
          <div className="text-2xl font-bold text-yellow-300">{store.draftCount}</div>
          <div className="text-sm text-slate-400">Drafts</div>
        </div>
        <div className="bg-slate-800/50 border border-slate-600/50 rounded-lg p-4">
          <div className="text-2xl font-bold text-blue-300">{store.recentCount}</div>
      ...
      {/* Create Post Form */}
      <form onSubmit={handleCreatePost} className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-6 space-y-4">
      ...
      <div className="space-y-4">
        {store.filteredPosts.map((post) => (
          <div key={post.id} className="bg-slate-800/30 border border-slate-700/50 rounded-lg p-6">
      ...
      <div className="mt-8 pt-6 border-t border-slate-700/50">
        <div className="text-xs text-slate-500 text-center space-y-1">
          <div>
            🧠 Custom Store • 🔄 Custom Transformer • 🔍 Advanced Search & Filtering
          </div>
          <div>
