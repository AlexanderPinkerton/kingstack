...
import {
  useStreamingCheckboxOptimisticStore,
  createStreamingCheckboxQuery,
} from "@/stores/streamingCheckboxStore";
...
export const useStreamingCheckboxes = (config: StreamingConfig) => {
  ...
  const streamingQuery = useQuery(
    createStreamingCheckboxQuery(config.totalItems, config.chunkSize),
  );
  ...
  useEffect(() => {
    if (streamingQuery.data && streamingQuery.data.length > 0) {
      console.log(
        `Processing ${streamingQuery.data.length} streamed checkboxes`,
      );
      ...
      console.log(
        `Added ${uiData.length} checkboxes to store. Total: ${optimisticStore.store.count}`,
      );
    }
    ...
    const handleRealtimeUpdate = (data: any) => {
      ...
      if (data.type === "checkbox_update") {
        ...
        if (event === "INSERT" || event === "UPDATE") {
          ...
          console.log(
            "Updated optimistic store with realtime data:",
            uiCheckbox,
          );
        } else if (event === "DELETE") {
          // Remove from optimistic store
          const checkboxToRemove = optimisticStore.store.list.find(
            (cb) => cb.index === checkbox.index,
          );
          if (checkboxToRemove) {
            runInAction(() => {
            ...
            console.log(
              "Removed checkbox from optimistic store:",
              checkbox.index,
            );
          }
        }
  ...
  const handleCheckboxChange = useCallback(
    (index: number, checked: boolean) => {
      ...
      const existingCheckbox = optimisticStore.store.list.find(
        (cb) => cb.index === index,
      );
  ...
  const getCheckbox = useCallback(
    (index: number) => {
      return optimisticStore.store.list.find(
  ...
  return {
    ...
    streamProgress: streamingQuery.data
      ? Math.min(100, (streamingQuery.data.length / config.totalItems) * 100)
      : 0,
