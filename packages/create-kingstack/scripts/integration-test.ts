...
async function runTest(test: TestCase): Promise<boolean> {
  ...
  try {
    const result =
      await $`docker run --rm -v ${REPO_ROOT}:/app -w /app node:24.18-alpine sh -c ${dockerCommand}`.quiet();
    ...
    if (test.expectedToFail) {
      console.log(
        pc.green("✓ PASSED") +
          ` (expected failure, got exit code ${error.exitCode})`,
      );
      return true;
    } else {
      console.log(
        pc.red("✗ FAILED") +
          ` (expected success, got exit code ${error.exitCode})`,
      );
      return false;
    }
  }
...
main().catch((error: unknown) => {
  console.error(pc.red("Integration test runner failed:"), error);
  process.exitCode = 1;
});
