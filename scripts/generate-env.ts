...
async function main() {
    ...
    console.log(`🔧 Generating .env files for environment: ${envArg}\n`);
    ...
    if (!existsSync(valuesPath)) {
        ...
        console.error(`\n💡 Tip: Copy secrets/example.ts to ${valuesPath} and fill in your values.`);
        process.exit(1);
    }
    ...
    if (projectErrors.length > 0) {
        console.error("❌ Project configuration errors:");
    ...
    for (const [projectName, config] of Object.entries(schema.projects)) {
        ...
        console.log(`✅ Generated ${destPath} (${config.keys.length} keys)`);
    }

    console.log(`\n🎉 Successfully generated .env files for ${envArg} environment!`);
}
