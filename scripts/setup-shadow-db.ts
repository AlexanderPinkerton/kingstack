...
async function main() {
  ...
  try {
    ...
    if (!containerId) {
      console.error(
        `❌ Could not find container named '${containerNameTarget}'`,
      );
  ...
  try {
    run(
      `docker exec ${containerId} psql -U postgres -c "DROP DATABASE IF EXISTS shadow_db WITH (FORCE);"`,
    );
    run(
      `docker exec ${containerId} psql -U postgres -c "CREATE DATABASE shadow_db;"`,
    );
  ...
  try {
    run(
      `docker exec ${containerId} psql -U postgres -d shadow_db -c "${resetSql.replace(/\n/g, " ")}"`,
    );
    console.log("✅ Public schema reset.");
  } catch {
    console.warn(
      "⚠️  Some permission grants may have warned, but schema is reset.",
    );
  }
...
main().catch((error: unknown) => {
  console.error("❌ Shadow database setup failed:", error);
  process.exitCode = 1;
});
